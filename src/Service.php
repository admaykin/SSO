<?php
namespace Invisio\SSO;
use Incvisio\SSO\NotAttachedException;
use Incvisio\SSO\Exception;
use Incvisio\Validation;

/**
 * Single sign-on service.
 *
 * The service lives on the website visited by the user. The service doesn't have any user credentials stored. Instead it
...
class Service
{
    ...
    /**
     * Identifier, given by SSO provider.
     * @var string
     */
    public $service;

    /**
     * Secret key, given by SSO provider.
     * @var string
     */
    protected $secret;
     ...
     * @param string $service Identifier, given by SSO provider.
     * @param string $secret Secret key, given by SSO provider.
     */
    public function __construct($url, $service, $secret)
    {
        if (!$url) throw new \InvalidArgumentException("SSO server URL not specified");
        if (!$service) throw new \InvalidArgumentException("SSO service id not specified");
        if (!$secret) throw new \InvalidArgumentException("SSO service secret not specified");
     ...
     * Get the cookie name.
     *
     * Note: Using the service name in the cookie name.
     * This resolves issues when multiple services are on the same domain.
     *
     * @return string
     */
    protected function getCookieName()
    {
        return 'sso_token_' . preg_replace('/[_\W]+/', '_', strtolower($this->service));
    }
    ...
    protected function getSessionId()
    {
        ...
        return "SSO-{$this->service}-{$this->token}-$checksum";
    }
     ...
     *
     * Only services marked trused can collect and send the user's credentials. Other services should omit $username and
     * $password.
     *
